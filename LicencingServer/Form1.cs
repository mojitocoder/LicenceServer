...
namespace LicencingServer
{
    public partial class Form1 : Form
    {
        ...
        private void btnActivate_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtAuthentication.Text))
            {
                MessageBox.Show("Authentication code is empty. Please enter an authentication code to generate the activation code", "Authentication Code Needed", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                var activationResult = new Activation().Activate(txtAuthentication.Text);
                if (activationResult == null) //invalid authentication code
                {
                    MessageBox.Show("Invalid authentication code.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    txtActivationCode.Text = activationResult.ActivationCode;
                    txtMac.Text = activationResult.MAC;
                }
            }
        }
...
<Exponent>AQAB</Exponent>
	...
	<D>QBmsJG1tE+nyO1MxeL2Mc8JYeLWrro//BHTA4kw0a0OBY90jYo6nOle492H1x4pDrYEA5zhZinnin+29BdVRL6uAy+XevI8Tu0INVsaLTIBoHQf1eMv/U8jNDNYxG4g9FyycWqtFW53vc8pmTjGgtcE9+O3YpSrU1vK2UNeVeaE=</D>
        ...
        private void ProgrammaticRsaKeys()
        {
        ...
        {
            using (RSACryptoServiceProvider cipher = new RSACryptoServiceProvider())
            {
                cipher.FromXmlString(privateKey);
                byte[] original = cipher.Decrypt(Convert.FromBase64String(cipherText), true);
    ...
    {
        public ActivationResult Activate(string authenticationCode)
        {
            try
            {
                var secret = new Encryption().Decrypt(authenticationCode);
                return new ActivationResult
                {
                    ActivationCode = secret.Split(':')[0],
                    MAC = secret.Split(':')[1]
                };
            }
            catch (Exception)
